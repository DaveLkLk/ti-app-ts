import { modulePagesManager } from "../events/_configOLD";
...
export function managerShowApp(isLoaded: boolean, typeModule: string, obj: DisplayElementApp){
    // params object
    // isSavedStorage: true o false si es la primera vez que se ejecuta
    // id: id del button toggle - string
    // nav: lista de apps en APPLICATIONS - UL
    // container: lista de opciones de aplicaciones - UL
    // console.log(obj);

    if(isLoaded && typeModule === modulePagesManager.pages.name){
        validateExistItemStorage()
        setDefaultStorageItem()
        changeDisplayElementApp(true, obj)
        return
    }
    if(!isLoaded && typeModule === modulePagesManager.pages.name){
        validateExistItemStorage()

        console.log(isLoaded+" isloaded es falso bro");
    }
}
